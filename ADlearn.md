...
在垂直分割页面下，使用该功能选中PCB上的元件，可以自动打开对应原理图；反之亦然。

### 元件推挤

PCB布局时，移动元器件有三种情况：1、拖拽元器件时推挤其他器件；2、拖拽元器件时无法放置在已有元件上；3、拖拽元器件时忽略规则，可以重叠，但会报错。

设置方法：工具>>优先选项>>PCB Editor>>Interactive Routing>>拖拽>>元器件推挤，该选项下Push、Avoid、Ignore分别对应上述三种情况。
